...
class TestIrbankBsScraper:
    ...
    def test_empty_detailed_parse_is_marked_as_no_data(self, db_conn: sqlite3.Connection) -> None:
        upsert_stock(db_conn, "7699", "No Data Inc.", "", "")
        db_conn.commit()

        client = _FakeClient({
            _SUMMARY_URL.format(ticker="7699"): _FakeResponse(_overview_html()),
            _DETAIL_URL_03: _FakeResponse("<html><body><h1>財務状況</h1></body></html>"),
        })
    ...
    def test_missing_annual_detail_link_is_recorded_as_status_row(
        ...
        upsert_stock(db_conn, "4452", "No Detail Link Inc.", "", "")
        db_conn.commit()

        client = _FakeClient({
            _SUMMARY_URL.format(ticker="4452"): _FakeResponse(_overview_html(include_annual_link=False)),
        })
        ...
        assert get_existing_tickers(db_conn, "irbank_bs") == {"4452"}
    ...
    def test_fetch_error_is_recorded_and_retried_on_future_default_runs(
        ...
        failing_client = _FakeClient({
            _SUMMARY_URL.format(ticker="9914"): _FakeResponse(
                None,
