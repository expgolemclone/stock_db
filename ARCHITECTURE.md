...
- `parse_financials(path)` — BS / PL / CF / dividend / forecast / shares outstanding の正規化。inventories は内部で `parse_inventories` を呼び出し、他項目は候補マッチングで直接 fact を検索。主表の無次元 fact を優先し、無い場合のみ `NonConsolidatedMember` 単独の非連結総額 context を fallback として使う。shares outstanding（発行済株式数）は `shares` 単位の fact を別バケットで抽出し BS items に格納する
...
- PyO3 関数 (`parse_inventories`, `parse_financials`) は `py.allow_threads()` で GIL を解放し、Python 側の `ThreadPoolExecutor` からマルチスレッド並列実行を可能にする

Python 側は `xbrl_bs_parser.py` / `xbrl_financials_parser.py` が Rust 拡張の薄いラッパーとして機能し、CLI や DB 書き込み層は Python のまま。
...
| `updater` | `download_latest_daily_file` と `ingest_daily_prices` を組み合わせ、DB初期化・commit・結果返却を行う public API |
