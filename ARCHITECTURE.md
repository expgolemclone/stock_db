...
stock_db/
  ...
  var/
    db/stocks.db           # SQLite データベース（Git LFS, Actionsから日次更新）
    raw/edinet/
...
- 手動検証時は `gh workflow run update-stooq-prices.yml --repo expgolemclone/stock_db --ref main` で `workflow_dispatch` を発火し、run が `queued` または `in_progress` になれば起動確認完了とみなす
- Stooq CAPTCHA OCR は DejaVu / FreeSans / Liberation Sans 系のフォントを前提とするため、GitHub Actions runner に該当フォントが存在しないと `No OCR fonts found for Stooq CAPTCHA solver` で失敗する
- 更新後は `sqlite3 var/db/stocks.db "PRAGMA wal_checkpoint(TRUNCATE);"` を実行し、WAL の内容を `stocks.db` 本体へ反映してから commit/push する
- workflow は `var/db/stocks.db` のみを更新対象とし、`stocks.db-wal` / `stocks.db-shm` や `var/raw/stooq/` は追跡しない
