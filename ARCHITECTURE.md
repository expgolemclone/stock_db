...
stock_db/
  ...
  rust/                    # Rust crate — XBRL パーサコア (PyO3 + maturin)
    Cargo.toml
    src/
      lib.rs               # PyO3 モジュール定義・公開関数 (parse_inventories, parse_financials)
  ...
  src/stock_db/            # Python パッケージ
    _edinet_xbrl.cpython-*.so  # maturin ビルド済み Rust 拡張
...
EDINET XBRL パーサの核心を Rust (PyO3 + maturin) で実装。`rust/` クレートを `stock_db._edinet_xbrl` として Python に公開。

- `parse_inventories(path)` — 棚卸資産総額の抽出。direct total → calculation linkbase → presentation linkbase → component sum の優先度チェーンで集計
- `parse_financials(path)` — BS / PL / CF / dividend / forecast / shares outstanding / preferred-share flag の正規化。inventories は内部で `parse_inventories` を呼び出し、他項目は候補マッチングで直接 fact を検索。主表の無次元 fact を優先し、無い場合のみ `NonConsolidatedMember` 単独の非連結総額 context を fallback として使う。shares outstanding（発行済株式数）は `shares` 単位の fact を別バケットで抽出し BS items に格納する
- `parse_share_classes(path)` — `ClassesOfSharesAxis` と提出者別タクソノミの種類株式別発行済株式数 fact から、普通株・優先株・その他種類株の明細を抽出する。優先株判定は member / concept / label の `PreferredShares` 又は `優先株式` に基づく
...
- Rayon (`par_iter`) で文書単位の XML 抽出を並列化。context/unit/nsmap/fact の抽出は文書ごとに独立しており、並列実行後に結果を逐次マージする。`store_fact_buckets` の HashMap 書き込みは競合を避けるため逐次のまま
...
- PyO3 関数 (`parse_inventories`, `parse_financials`, `parse_share_classes`) は `py.allow_threads()` で GIL を解放し、Python 側の `ThreadPoolExecutor` からマルチスレッド並列実行を可能にする

Python 側は `xbrl_bs_parser.py` / `xbrl_financials_parser.py` / `xbrl_share_classes_parser.py` が Rust 拡張の薄いラッパーとして機能し、CLI や DB 書き込み層は Python のまま。
...
`parse_xbrl_financials` は `sec_reports` 上の同一 ticker の全 artifact を fiscal year 昇順で畳み込み、後続 filing の同一 `(period, statement, item_name)` と `(period, class_name)` を優先する。その後 `financial_items` の同一 ticker に対する `irbank` / `irbank_bs` / `irbank_forecast` / `xbrl_bs` / 旧 `edinet_xbrl` を削除し、再構築した rows を `source=edinet_xbrl` で一括登録する。種類株明細は `share_classes` の同一 ticker / source を置換する。`financial_items` の主キーは `(ticker, period, statement, item_name)` で `source` を含まないため、source 単位ではなく ticker 単位で置換する。`--skip-existing` はデフォルトで有効であり、既存 `edinet_xbrl` を持つ ticker を skip する。再パースが必要な場合だけ `--force` を使う。`--jobs N`（既定値 4、`cli_defaults.toml` で設定）で ticker 単位の並列パースを行う。Rust 拡張の `py.allow_threads()` で GIL を解放するため、Python `ThreadPoolExecutor` からマルチスレッドで Rust パース本体を並列実行できる。SQLite 書き込みと commit は main thread のみで行う。シリアル・並列ともに `InventoriesTagMismatchError` のみを捕捉し、予期しない例外は伝播させる。
...
他プロジェクトからは `run_stooq_price_update_command()` を使う。これは `stock_db` プロジェクトルートを cwd にして `uv run scrape-stooq-prices` を実行し、CLI と同じ browser service 起動経路を使う。

`storage.prices.is_stooq_price_update_required` は、`prices` の最新価格日付と JPX 営業日カレンダーを比較し、最新価格日の翌日から当日までに JPX 営業日が1日以上ある場合だけ更新を要求する。JPX の市場休日は `config/jpx_market_holidays.toml` に年別で明示し、判定対象年が未定義の場合は例外で止める。土日だけの判定には fallback しない。
