...
stock_db/
  ...
  src/stock_db/            # Python パッケージ
    ...
    sources/edinet/
      ...
      xbrl_share_classes_parser.py # 薄い Python ラッパー — Rust parse_share_classes を呼び出し
    sources/stooq/
...
- `parse_financials(path)` — BS / PL / CF / dividend / forecast / shares outstanding / preferred-share flag の正規化。inventories は内部で `parse_inventories` を呼び出し、他項目は候補マッチングで直接 fact を検索する。CF 候補は JPPFS 系と IFRS 系の alias を正規化する。主表の無次元 fact を優先し、無い場合のみ `NonConsolidatedMember` 単独の非連結総額 context を fallback として使う。shares outstanding（発行済株式数）は `shares` 単位の fact を別バケットで抽出し BS items に格納する
...
| `xbrl_bs_parser` | Rust 拡張 (`_edinet_xbrl.parse_inventories`) の薄い Python ラッパー。`is_valid_xbrl_text` / `is_valid_xbrl_path` のバリデーション補助関数は Python 側に残存 |
| `xbrl_financials_parser` | Rust 拡張 (`_edinet_xbrl.parse_financials`) の薄い Python ラッパー |
| `xbrl_share_classes_parser` | Rust 拡張 (`_edinet_xbrl.parse_share_classes`) の薄い Python ラッパー |
