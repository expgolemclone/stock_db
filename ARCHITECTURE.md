...
- `parse_financials(path)` — BS / PL / CF / dividend / forecast の正規化。inventories は内部で `parse_inventories` を呼び出し、他項目は候補マッチングで直接 fact を検索。主表の無次元 fact を優先し、無い場合のみ `NonConsolidatedMember` 単独の非連結総額 context を fallback として使う
