...
- 手動検証時は `gh workflow run update-stooq-prices.yml --repo expgolemclone/stock_db --ref main` で `workflow_dispatch` を発火し、`Update Stooq prices` / `Checkpoint SQLite WAL` / `Commit updated database` がすべて success になることを確認する
