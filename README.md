...
- `scrape-edinet-historical` は discovery の途中結果、API 失敗日、download/sync の docID 別ステータスを `var/raw/edinet/discovery/*.json` に保存し、同じ日付範囲の再実行では完了済み日付と処理済み docID を再走査しない
...
- Stooq 更新は `stock_db.sources.stooq.update_stooq_daily_prices()` として API 化されており、CLI 以外のアプリケーションからも同じ処理を呼び出す
...
- `config/jpx_market_holidays.toml` — JPX 公式 Market Holidays に基づく市場休日の年別定義
